...
public class ConnectHandler extends AbstractHandler implements IElementUpdater {
	...
	/** 轮询时间间隔 */
	public static final int WAITFOR_DEVICE_INTERVAL = 300;
	/** 轮询超时时间 */
	public static final int WAITFOR_DEVICE_TIMEOUT = 8000;
        ...
        Job job = new Job("Connect to adb over wifi") {
            ...
            protected IStatus run(IProgressMonitor monitor) {
                try {
                    ...
                    String message;
                    if (deviceIpAddress.isEmpty() || devicePortNumber.isEmpty()) {
                        message = "Unable to connect to device. Is ADB Wireless " +
                    	...
                    	message = "Waiting for device is timeout.";
    ...
    private boolean waitForDevice(final String devSerial,
        ...
        int timeLeft = WAITFOR_DEVICE_TIMEOUT; // TODO: Make this an option?
        ...
        try {
            ...
            worker.start();
             ...
             while (!monitor.isCanceled() && !worker.isComplete()) {
                try {
                    worker.join(WAITFOR_DEVICE_INTERVAL);
                    timeLeft -= WAITFOR_DEVICE_INTERVAL;
                    if (timeLeft < 0) {
                        worker.interrupt();
                        break;
                    }
